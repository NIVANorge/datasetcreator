...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gmi="http://www.isotc211.org/2005/gmi" xmlns:srv="http://www.isotc211.org/2005/srv" xmlns:gmx="http://www.isotc211.org/2005/gmx" xmlns:gsr="http://www.isotc211.org/2005/gsr" xmlns:gss="http://www.isotc211.org/2005/gss" xmlns:gts="http://www.isotc211.org/2005/gts" xmlns:gml="http://www.opengis.net/gml/3.2" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:nc="http://www.unidata.ucar.edu/namespaces/netcdf/ncml-2.2" exclude-result-prefixes="nc">
    ...
    <xsl:variable name="title_no" as="xs:string*" select="(/nc:netcdf/nc:attribute[@name='title_no']/@value)"/>
    <xsl:variable name="summary_no" as="xs:string*" select="(/nc:netcdf/nc:attribute[@name='summary_no']/@value)"/>
    ...
    <xsl:variable name="iso_topic_category" as="xs:string*" select="(/nc:netcdf/nc:attribute[@name='iso_topic_category']/@value)"/>
    <!--jmaurer-->
    ...
    <xsl:variable name="geospatial_lat_units" as="xs:string*" select="(/nc:netcdf/nc:group[@name='CFMetadata']/nc:attribute[@name='geospatial_lat_units']/@value)"/>
    <xsl:variable name="geospatial_lon_units" as="xs:string*" select="(/nc:netcdf/nc:group[@name='CFMetadata']/nc:attribute[@name='geospatial_lon_units']/@value)"/>
    <xsl:variable name="temporalUnits" as="xs:string*" select="(/nc:netcdf/nc:group[@name='CFMetadata']/nc:attribute[@name='time_coverage_units']/@value)"/>
    <!-- Modified on 2014-04-19 -->
    <xsl:variable name="verticalUnits" as="xs:string*" select="(/nc:netcdf/nc:group[@name='CFMetadata']/nc:attribute[@name='geospatial_vertical_units']/@value)"/>
                ...
                <gmd:MD_DataIdentification id="DataIdentification">
                                ...
                                <gmd:PT_FreeText>
                                    <gmd:textGroup>
                                      <gmd:LocalisedCharacterString locale="#locale-nob">
                                        <xsl:value-of select="$title_no[1]"/>
                    ...
                    <gmd:abstract>
                        ...
                        <gmd:PT_FreeText>
                            <gmd:textGroup>
                              <gmd:LocalisedCharacterString locale="#locale-nob">
                                <xsl:value-of select="$summary_no[1]"/>
                              </gmd:LocalisedCharacterString>
                            </gmd:textGroup>
                          </gmd:PT_FreeText>
                    </gmd:abstract>
                    ...
                    <xsl:if test="count($keywords)">
                        <xsl:for-each select="tokenize($keywordsVocabulary[1],',')">
                            <xsl:variable name="vocShort" select="substring-before(.,':')"/>
                            <xsl:variable name="vocLong" select="substring-before(substring-after(.,':'), ':')"/>
                            <xsl:variable name="vocUrl" select="substring-after(substring-after(.,':'), ':')"/>
                            <gmd:descriptiveKeywords>
                                <gmd:MD_Keywords>
                                    <xsl:for-each select="tokenize($keywords[1],',')">
                                        <xsl:variable name="keywordVoc" select="substring-before(.,':')"/>
                                        <xsl:variable name="keyword" select="substring-after(.,':')"/>
                                        <xsl:if test="$keywordVoc=$vocShort">
                                            <gmd:keyword>
                                                <gco:CharacterString>
                    ...
                    <gmd:topicCategory>
                        <xsl:choose>
                            <xsl:when test="count($iso_topic_category)">
                                <gmd:MD_TopicCategoryCode>
                                    <xsl:value-of select="$iso_topic_category[1]"/>
                                </gmd:MD_TopicCategoryCode>
                            </xsl:when>
                            <xsl:otherwise>
